...
export default function Dashboard() {
  ...
  // Fetch topics based on user's degree stored in localStorage (e.g., from AcademicsForm)
  useEffect(() => {
    const fetchTopics = async () => {
      try {
        const res = await fetch("/degreesAndSubtopics.json");
        const data = await res.json();
        // Assume the user's degree is stored in localStorage under "userDegree"
        const userDegree = localStorage.getItem("userDegree");
        if (userDegree) {
          const course = data.degree_courses.find(
            (course: { name: string; subtopics: string[] }) => course.name === userDegree
          );
          if (course) {
            setAvailableTopics(course.subtopics);
          }
        }
      } catch (err) {
    ...
    <div className={`${theme === "dark" ? "dark" : ""}`}>
      <div className="min-h-screen bg-gray-100 dark:bg-gray-900 dark:text-gray-100">
        ...
        <main className="max-w-4xl mx-auto p-8">
          <h1 className="text-3xl font-bold mb-4">Welcome to Your Study Buddy</h1>
          <p className="mb-6 text-gray-700 dark:text-gray-300">
            Get connected with a study buddy to help you with your studies. Select your
            topics and purpose of study. Find the perfect student study partner for exam
            prep, revision, learning new material, mentoring, or advice.
          </p>
          <button
            onClick={handleGetStarted}
            className="bg-gray-600 text-white px-6 py-3 rounded hover:bg-blue-700 transition"
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {availableTopics.length > 0 ? (
                    ...
                    <p>No topics available for the selected degree.</p>
                  )}
                </div>
              </div>
