...
const UserNavLinks = ({ userNavLinks, pathName }: UserNavLinksProps) =>
    userNavLinks.map((link) => (
        <DropdownMenuItem key={link.title}>
            <Link href={link.url} className={cn("py-2 text-sm transition-colors", pathName === link.url ? "text-black dark:text-white" : "text-muted-foreground")}>
                {link.title}
            </Link>
        </DropdownMenuItem>
    ));
    ...
    return (
        <DropdownMenu>
            <DropdownMenuTrigger className="flex items-center gap-4 cursor-pointer">
                <Avatar>
                    <AvatarImage src={user?.image || (user && `https://api.dicebear.com/9.x/initials/svg?seed=${user?.name}`)} />
            ...
            <DropdownMenuContent className="w-30 mt-2   ">
                <div className=" flex gap-2 items-center p-2 ">
                    {user && <span > <p className="text-xs "> Logged in as </p> <b>{user?.name}</b></span>}
                    <DropdownMenuItem>
                        <AuthLink isSignedIn={user ? true : false} />
                    </DropdownMenuItem>
                </div>
                <div className="p-3 hover:bg-gray-700 rounded-lg transition duration-300">
                    <Link href="/settings">
